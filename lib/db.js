...
class Db {
  ...
  getImage (id, callback) {
    ...
    let tasks = co.wrap(function * () {
      ...
      if (!image) {
        return Promise.reject(new Error(`image ${image} not found`))
      }
  ...
  getImages (callback) {
    ...
    let tasks = co.wrap(function * () {
      let conn = yield connection
      let image = yield r.db(db).table('images').orderBy(r.desc('createdAt')).run(conn)

      let result = yield image.toArray()
